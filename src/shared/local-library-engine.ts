...
import { buildLocalLibrarySnapshot, createDefaultLocalAppSettings, defaultFolderNameForSourceType, defaultTemplateIdForSourceType } from "./local-library-defaults.js";
import type {
  FolderRecord,
  LocalAppSettings,
  LocalCardRecord,
...
import { buildReviewFeedbackMessage, buildReviewScheduleUpdate } from "./review-scheduler.js";

export function mergeLocalLibraryData(raw: Partial<LocalLibraryData>, defaults: LocalLibraryData): LocalLibraryData {
...
export function saveSettingsInLibrary(data: LocalLibraryData, settings: Partial<LocalAppSettings>) {
  const next = {
    ...data,
  ...
  return {
    data: next,
    snapshot: buildLocalLibrarySnapshot(next),
  };
}

export function saveCardInLibrary(data: LocalLibraryData, input: SaveCardInput) {
  const now = new Date().toISOString();
  const folderName = input.form.folder.trim() || defaultFolderNameForSourceType(input.form.sourceType);
  const working = cloneLocalLibraryData(data);
  const folder = ensureFolder(working, folderName, input.structuredData.sourceType, now);
  const template = ensureTemplate(working, folder.templateId, input.structuredData.sourceType, now);
  const existing = input.cardId ? working.cards.find((card) => card.id === input.cardId) : undefined;
  ...
  working.cards = [card].concat(working.cards.filter((item) => item.id !== card.id)).sort(sortCardsByUpdatedAt);
  working.settings = normalizeSettings({
    ...working.settings,
    folder: folder.name,
    deckTag: input.form.deckTag,
  ...
  return {
    data: working,
    card,
    snapshot: buildLocalLibrarySnapshot(working),
    message: existing ? "本地知识卡片已更新。" : "已保存到本地知识库。",
  };
}

export function saveFolderInLibrary(data: LocalLibraryData, input: SaveFolderInput) {
  ...
  const now = new Date().toISOString();
  const working = cloneLocalLibraryData(data);
  const existing = input.id ? working.folders.find((item) => item.id === input.id) : undefined;
  ...
  working.cards = working.cards.map((card) =>
...
export function saveTemplateInLibrary(data: LocalLibraryData, input: SaveTemplateInput) {
  const name = input.name.trim();
  ...
  const now = new Date().toISOString();
  const working = cloneLocalLibraryData(data);
  const existing = input.id ? working.templates.find((item) => item.id === input.id) : undefined;
...
export function reviewCardInLibrary(data: LocalLibraryData, input: ReviewCardInput) {
  ...
  working.cards = [next].concat(working.cards.filter((card) => card.id !== next.id)).sort(sortCardsByUpdatedAt);
...
export function normalizeSettings(input?: Partial<LocalAppSettings>): LocalAppSettings {
...
function cloneLocalLibraryData(data: LocalLibraryData): LocalLibraryData {
  return {
    version: data.version,
    settings: { ...data.settings },
    folders: data.folders.map((item) => ({ ...item })),
    templates: data.templates.map((item) => ({ ...item, enabledFields: [...item.enabledFields] })),
    cards: data.cards.map((item) => ({
      ...item,
      keywords: [...item.keywords],
      flashcards: item.flashcards.map((card) => ({ ...card })),
      structuredData: {
        ...item.structuredData,
        keywords: [...item.structuredData.keywords],
        flashcards: item.structuredData.flashcards.map((card) => ({ ...card })),
      },
    })),
  };
}
